...
if len(sys.argv) < 2:
    ...
    while 1:
        ...
        lower_color = np.array(
            [
                cv2.getTrackbarPos("H_low", "image"),
                cv2.getTrackbarPos("S_low", "image"),
                cv2.getTrackbarPos("V_low", "image"),
            ]
        )
        upper_color = np.array(
            [
                cv2.getTrackbarPos("H_up", "image"),
                cv2.getTrackbarPos("S_up", "image"),
                cv2.getTrackbarPos("V_up", "image"),
            ]
        )

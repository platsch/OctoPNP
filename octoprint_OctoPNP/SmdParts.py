...
class SmdParts:
    ...
    def getPartHeight(self, partnr):
        return float(
            self._et.find("./part[@id='" + str(partnr) + "']/size").get("height")
        )
    ...
    def getPartDestination(self, partnr):
        x = float(
            self._et.find("./part[@id='" + str(partnr) + "']/destination").get("x")
        )
        y = float(
            self._et.find("./part[@id='" + str(partnr) + "']/destination").get("y")
        )
        z = float(
            self._et.find("./part[@id='" + str(partnr) + "']/destination").get("z")
        )
        orientation = float(
            self._et.find("./part[@id='" + str(partnr) + "']/destination").get(
                "orientation"
            )
        )
                ...
                if result:
                    if part.find("shape") is not None:
                        for elem in part.find("shape"):
                            result, msg = self._sanitizeAttribute(
                                part, elem, ["x", "y"], float
                            )
                # pads
                if result:
                    if part.find("pads") is not None:
                        for elem in part.find("pads"):
                            result, msg = self._sanitizeAttribute(
                                part, elem, ["x1", "y1", "x2", "y2"], float
                            )
                # destination
                if result:
                    result, msg = self._sanitizeTag(
